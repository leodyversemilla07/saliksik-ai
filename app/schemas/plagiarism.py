...
class SimilarDocument(BaseModel):
    ...
    matched_segments: List[str] = Field(
        ...
        description="Sample text segments that matched"
    )
...
class PlagiarismResult(BaseModel):
    ...
    overall_similarity: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Highest similarity score found (0.0-1.0)"
    )
    ...
    unique_content_percentage: float = Field(
        ...
        ge=0.0,
        le=100.0,
        description="Percentage of content considered unique"
    )
...
class PlagiarismCheckRequest(BaseModel):
    ...
    threshold: Optional[float] = Field(
        ...
        ge=0.1,
        le=0.95,
        description="Similarity threshold (0.1-0.95)"
    )
