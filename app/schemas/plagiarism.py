...
class SimilarDocument(BaseModel):
    ...
    matched_segments: List[str] = Field(default_factory=list, description="Sample text segments that matched")
    original_filename: Optional[str] = Field(None, description="Original filename of the similar document")
...
class PlagiarismResult(BaseModel):
    ...
    overall_similarity: float = Field(..., ge=0.0, le=1.0, description="Highest similarity score found (0.0-1.0)")
    ...
    processing_time: float = Field(..., description="Time taken for plagiarism check in seconds")
    checked_against: int = Field(default=0, description="Number of documents compared against")
...
class PlagiarismCheckRequest(BaseModel):
    ...
    threshold: Optional[float] = Field(default=0.5, ge=0.1, le=0.95, description="Similarity threshold (0.1-0.95)")
...
class PlagiarismIndexStats(BaseModel):
    ...
    datasketch_available: bool = Field(..., description="Whether datasketch library is available")
    xxhash_available: bool = Field(..., description="Whether xxhash library is available")
