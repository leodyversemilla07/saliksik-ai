...
class ReviewerCreate(BaseModel):
    ...
    expertise_description: Optional[str] = Field(
        None,
        max_length=1000,
        description="Free-text description of expertise"
    )
...
class ReviewerUpdate(BaseModel):
    ...
    expertise_keywords: Optional[List[str]] = Field(
        None,
        max_length=20,
        description="Updated expertise keywords"
    )
    expertise_description: Optional[str] = Field(
        None,
        max_length=1000,
    ...
    orcid_id: Optional[str] = Field(
        None,
        description="Updated ORCID identifier"
    )
    is_available: Optional[bool] = Field(
        None,
        description="Whether accepting new reviews"
    )
    max_assignments: Optional[int] = Field(
        None,
        ge=1,
        le=20,
        description="Updated max assignments"
    )
...
class ReviewerAssignRequest(BaseModel):
    ...
    send_invitation: bool = Field(
        default=True,
        description="Whether to send an invitation notification"
    )
...
class ReviewerMatchStatus(BaseModel):
    ...
    status: str = Field(
        ...,
        description="New status: accepted, declined, completed"
    )
