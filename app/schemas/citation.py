...
class CitationIssue(BaseModel):
    ...
    issue_type: str = Field(..., description="Type of issue: format, missing, orphan, outdated")
...
class CitationAnalysisResult(BaseModel):
    ...
    format_detected: str = Field(..., description="Detected citation format (apa, mla, ieee, chicago, unknown)")
    ...
    missing_in_text: List[str] = Field(default_factory=list, description="References not cited in the text body")
    orphan_citations: List[str] = Field(
        default_factory=list,
        description="In-text citations without matching references",
    )
    issues: List[CitationIssue] = Field(default_factory=list, description="All issues found during analysis")
