...
class InTextCitation(BaseModel):
    ...
    authors: List[str] = Field(
        default_factory=list, description="Author names referenced"
    )
...
class CitationAnalysisResult(BaseModel):
    ...
    valid_citations: int = Field(
        ..., description="Number of properly formatted citations"
    )
    ...
    average_citation_age: float = Field(
        ..., description="Average age of citations in years"
    )
    oldest_citation_year: Optional[int] = Field(
        None, description="Year of oldest citation"
    )
    newest_citation_year: Optional[int] = Field(
        None, description="Year of newest citation"
    )
    missing_in_text: List[str] = Field(
        default_factory=list, description="References not cited in the text body"
    )
    ...
    issues: List[CitationIssue] = Field(
        default_factory=list, description="All issues found during analysis"
    )
