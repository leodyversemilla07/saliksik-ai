...
async def get_auth_headers(client, username: str, email: str):
    ...
    await client.post("/api/v1/auth/register", json={"username": username, "email": email, "password": "password123"})
...
async def test_create_duplicate_reviewer_profile(client, db_session):
    ...
    await client.post(
        "/api/v1/reviewers/", json={"expertise_keywords": ["data science"], "max_assignments": 3}, headers=headers
    )

    # Try to create second profile
    response = await client.post(
        "/api/v1/reviewers/", json={"expertise_keywords": ["statistics"], "max_assignments": 2}, headers=headers
    )
...
async def test_get_my_reviewer_profile(client, db_session):
    ...
    await client.post(
        "/api/v1/reviewers/",
        json={"expertise_keywords": ["bioinformatics"], "institution": "Research Institute"},
        headers=headers,
    )
...
async def test_update_reviewer_profile(client, db_session):
    ...
    # Create profile
    await client.post(
        "/api/v1/reviewers/", json={"expertise_keywords": ["chemistry"], "is_available": True}, headers=headers
    )
...
async def test_list_reviewers(client, db_session):
    ...
    await client.post("/api/v1/reviewers/", json={"expertise_keywords": ["physics"]}, headers=headers)
...
async def test_reviewers_requires_auth(client):
    ...
    response = await client.post("/api/v1/reviewers/", json={"expertise_keywords": ["test"]})
    assert response.status_code == 401
