...
async def test_pre_review_submission(client, db_session):
    ...
    await client.post(
        ...
        json={"username": "analysisuser", "email": "analysis@example.com", "password": "password123"},
    )

    login_res = await client.post("/api/v1/auth/login", data={"username": "analysisuser", "password": "password123"})
    ...
    response = await client.post(
        ...
        data={
            "manuscript_text": (
                "This is a long enough text to pass validation for the manuscript analysis system test case."
            )
        },
        headers=headers,
    )
