...
def upgrade() -> None:
    ...
    op.create_index("ix_manuscript_analyses_user_status", "manuscript_analyses", ["user_id", "status"], unique=False)

    # Composite index for listing analyses by date (pagination)
    op.create_index(
        "ix_manuscript_analyses_user_created", "manuscript_analyses", ["user_id", "created_at"], unique=False
    )

    # Index for finding available reviewers quickly
    op.create_index(
        "ix_reviewers_available_assignments", "reviewers", ["is_available", "current_assignments"], unique=False
    )

    # Composite index for reviewer match queries
    op.create_index("ix_reviewer_matches_analysis_status", "reviewer_matches", ["analysis_id", "status"], unique=False)

    # Composite index for reviewer's matches
    op.create_index("ix_reviewer_matches_reviewer_status", "reviewer_matches", ["reviewer_id", "status"], unique=False)

    # Index for match score ordering (finding best matches)
    op.create_index("ix_reviewer_matches_score", "reviewer_matches", ["analysis_id", "match_score"], unique=False)

    # Index for error tracking by type and date
    op.create_index("ix_processing_errors_type_date", "processing_errors", ["error_type", "created_at"], unique=False)
